...
import com.TaskManagement3.Client.IssueClient;
...
public class IntegrationService {
    ...
    public void handGitHubService(Map<String,Object>payLoad) {
    	...
    	for(Map<String,Object>commit:commitsCodes) {
    		String message=(String)commit.get("author");
    		String author= ((Map<String,String>)commit.get("author")).get("email");
    		
    		extractIssueKey(message).ifPresent(issueId-> {
    			issueClient.updateStatus(issueId, IssueStatus.DONE, author);
        		issueClient.addComments(issueId, author, "Closed By GitHub commint:"+message);
    ...
    public void handlejekinService(Map<String,Object>payLoad) {
    	
    	String jobName= (String)payLoad.get("jobs");
    	String status= (String)payLoad.get("status");
    	
    	extractIssueKey(jobName).ifPresent(issueId->{
    		if("Failuer".equals(status)) {
    		
    		
    		issueClient.addComments(issueId, "jenkin@system","Bild failed for job"+ jobName);
